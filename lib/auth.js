...
export async function getCurrentUser() {
  try {
    // КЛЮЧЕВОЕ: нет cookie сессии Supabase → это аноним/бот, Auth не дёргаем.
    // Header вызывает getCurrentUser на каждой странице; без этой проверки
    // каждый заход бота уходил в Auth и забивал инстанс (~900k запросов/сутки).
    const cookieStore = cookies();
    const hasAuthCookie = cookieStore
      .getAll()
      .some((c) => c.name.startsWith('sb-') && c.name.includes('auth-token'));
    if (!hasAuthCookie) return { user: null, profile: null };
