...
export const INT_SOURCES = ['freelancer', 'peopleperhour', 'guru'];

export async function runAllParsers() {
  console.log('[Parsers] Запуск...');

  const [fl, kwork, freelanceru, youdo, freelancer, pph, guru] = await Promise.allSettled([
    parseFL(),
    parseKwork(),
    parseFreelanceRu(),
    parseYoudo(),
  ...
  if (allProjects.length > 0) {
    const { data: inserted, error } = await db
      ...
      .select('id, source, title, description, category, budget_min, budget_max, currency, url, referral_url');
  ...
  // Раньше эта функция существовала в коде, но никогда не вызывалась —
  // из-за этого молча не работали ни личные уведомления премиум-юзерам,
  // ни постановка в очередь постинга в публичные Telegram-каналы (сама
  // очередь никем больше не наполнялась).
  if (newProjects.length > 0) {
    try {
      await sendNewProjectNotifications(newProjects);
    } catch (err) {
      console.error('[Telegram] Ошибка отправки уведомлений:', err.message);
    }
  }
